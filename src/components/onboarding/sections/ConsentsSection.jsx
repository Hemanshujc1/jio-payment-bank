...
const ConsentsSection = ({
  ...
  const isAllSelected =
    consents.length > 0 &&
    consents.every((c) => selectedConsents[c.consentTextCode]);
  ...
  const toggleAllConsents = () => {
    ...
    if (newState) {
      requestAnimationFrame(() => {
        if (consentContainerRef.current) {
          consentContainerRef.current.scrollTo({
            top: consentContainerRef.current.scrollHeight,
            behavior: "smooth",
          });
        }
      });
    ...
    <>
      <div className="w-full flex justify-start mb-4">
        <label className="flex items-center gap-3 cursor-pointer mt-2 select-none">
          <div className="cursor-pointer shrink-0" onClick={toggleAllConsents}>
            <div
              className={`w-5 h-5 border-2 border-primary flex items-center justify-center`}
            >
              {isAllSelected && (
                <IoMdCheckmark className="text-black text-lg" />
              )}
            </div>
          </div>
          <span
      ...
      <div
        ...
        className="w-full max-h-62.5 sm:max-h-75 overflow-y-auto pr-2 sm:pr-4 mb-2 border border-gray-200 rounded-lg p-3 sm:p-5 bg-gray-50/50 shadow-inner"
      >
        {consents.map((consentItem, index) => {
          const isSelected =
            selectedConsents[consentItem.consentTextCode] || false;
          ...
          return (
            <div
              key={consentItem.consentTextCode}
              className={`flex flex-col gap-4 sm:gap-2 ${
                isLast ? "mb-0" : "mb-6 sm:mb-5"
              }`}
            >
              <div className="flex items-start gap-3 sm:gap-4 group">
                ...
                >
                  <div
                    className={`w-5 h-5 shrink-0 border-2 ${
                      hasError ? "border-red-500" : "border-primary"
                    } flex items-center justify-center transition-colors group-hover:border-gray-900`}
                  >
              ...
              {hasError && (
                <span className="text-red-500 text-[11px] sm:text-[12px] font-medium ml-8 sm:ml-9">
                  Please agree to this consent
                </span>
              )}
            </div>
          );
